...
| 目的 | どこ | 仕組み |
| --- | --- | --- |
| 位置が入れ替わったことを見せる | テーマのつまみ、ナビの現在地、サイドバーの現在地、教材の目次（今いる章と読んでいる節）、実行計画の今の行 | `layoutId` で 1 つの要素を繋いで滑らせる |
| 現れた・消えたことを見せる | 教材の開閉、採点結果、ヒント、記述式の模範解答（4 枚を上から順に）、スキーマ一覧、模範解答の SQL（1 行ずつ） | `AnimatePresence` / `variants` の `staggerChildren` |
| 何が変わったかを見せる | 問題一覧のフィルタ（残った行が滑って詰まる・選択の下地が膨らむ）、数字（桁の輪が回る） | `layout` + `popLayout`、`useMotionValue` |
| どこを指しているかを示す | 教材の章カード（節が扇状にずれる）、前後送りの矢印、節の目盛り（近づいた所だけ伸びる） | `whileHover` / カーソルとの距離をバネに通す |
| 手順そのものを見せる | 実行計画を実行順に 1 歩ずつ、リセットの蓋が開く | 変形する図形（`d` の補間）と操作盤 |
...
込み入った動きは [Rare UI](https://www.rareui.com/)（MIT・[swamimalode07/rare-ui](https://github.com/swamimalode07/rare-ui)）から採っている。同じ Motion を使っているので考え方をそのまま持ち込めるが、**そのままは置いていない**。shadcn ではないので CLI は使わず、[src/components/rare/](src/components/rare/) に書き直してある。共通して変えたのは、配色をこの教材のトークンに寄せたこと・`cn` や `lucide-react` などの依存を外したこと・読み上げと「視差効果を減らす」設定への対応を足したこと。
...
| Code Block | 模範解答 | 色付けは入れない（CodeMirror と二重に持つと必ずずれる）。コピーの ✓ が描かれる所と行ごとの遅れだけを採る |
