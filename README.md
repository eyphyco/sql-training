...
問題ページの左に、7 フェーズを畳める目次を置いている。フェーズを開くとその中の問題が並び、クリックで移動できる。正解済みはチェック、いま解いている問題は帯で示す。

**作業領域の幅は減らしていない。** 問題文は 62rem で止めているので、器（1600px）との差が余白として空いていた。サイドバーはそこに入れてあり、SQL エディタと実行結果は今までどおり全幅のまま下に置く。
...
DOM 上は本文を先に置き、サイドバーは `lg:col-start-1 lg:row-start-1` で左へ回している。読み上げやタブ移動が目次 52 件から始まらないようにするため。
