    ...
    <div className="max-w-prose-wide space-y-5">
      ...
      <div>
        <div className="flex items-center gap-2 text-[12px]">
          ...
          <span className="tnum font-mono text-[10.5px] text-subtle">
            第 {lesson.phase} 章
          </span>
        </div>
      ...
      {/* 目次 */}
      <Card className="p-4">
        <p className="mb-2 text-[11.5px] font-medium text-muted">この章の内容</p>
        <ol className="space-y-1">
          {lesson.sections.map((s, i) => (
            <li key={s.id} className="flex items-baseline gap-2 text-[13px]">
              <span className="tnum font-mono text-[11px] text-subtle">{i + 1}.</span>
              <a href={`#${s.id}`} className="text-fg hover:text-accent">
                {s.title}
              </a>
            </li>
          ))}
