...
**押しても、つまみをつまんで動かしても切り替わる。** つまみは Motion の `drag` で動かし、離すといちばん近い区画にバネで吸い付く（`nearestIndex`）。端を越えて引いても `dragConstraints` で止まる。

切り替えた瞬間は**日の入りと日の出**にした。外れた側は下へ沈んで消え、選ばれた側は下から昇ってくる。ボタンを `overflow-hidden` の丸で切り抜いてあるので、その下辺が地平線になる。沈んだ側は見えないまま元の位置へ戻し、控えめな明るさで出し直す（ラベルとして残す必要があるため）。

つまみのカーソルは `grab` / `grabbing` をやめて `pointer` にしてある。この 2 つはブラウザが内蔵ビットマップで描くので、高 DPI で粗く見える。

### 配色の切り替えは色をつなぐ

白と黒が 1 フレームで入れ替わると目に痛いので、切り替えの間だけ色に時間をかける。常時 `*` に `transition` を掛けると操作のたびに重くなるため、`applyTheme()` が `:root` に `.theme-fading` を付け、終わったら外す。

`transform` と `opacity` は対象に入れない。Motion が動かしているものと二重に効いてしまうため。

**文字だけ短い（下地 200ms / 文字 140ms）。** 黒い文字と白い下地が入れ替わる以上、途中で必ず「どちらも中間の灰色」になる瞬間を通る。避けられないので、文字を先に着地させて短く抜ける。下地と文字のコントラスト比を実測して決めた。

| 下地 / 文字 | 最小 | 3:1 を下回る時間 |
| --- | --- | --- |
| 200 / 200ms | 1.13:1 | 39ms |
| **200 / 140ms** | **1.48:1** | 42ms |
| 320 / 170ms | 1.00:1 | 71ms |

長くするほど淡くなるが、読めない時間も比例して伸びる。200ms は「急変でなく、かつ抜けが速い」あたり。

「視差効果を減らす」設定でも止めない。ここで避けたいのは動きではなく明暗の急変で、一瞬で飛ばすほうがつらいため。

つまみを 1 つの要素にしたので、以前の「選択中のボタンの中に `layoutId` で置く」形はやめた。ボタンの中にあるとつかんで動かせないため。代わりに停止位置をボタンの実寸（`offsetLeft`）から測っている。余白やアイコンの大きさを変えても追従する。

- アイコンは `pointer-events: none` でつまみの上に描く。見た目は今までどおり、ポインタはつまみに通る
- 停止位置へ寄せるのは `x.set()` ではなく `animate()`。離した直後にドラッグ側の最終位置で上書きされてしまうため
- 「視差効果を減らす」設定では時間 0 で置く。`animate()` は `MotionConfig` の外なので、この判定だけ自分で持っている
...
線の太さは「画面上で何 px に見えるか」で決めている（`STROKE_PX = 1.2`）。viewBox は 24 固定なので `strokeWidth` を据え置くと**小さいアイコンほど線が細くなり**、1px を割ったところでにじむ（14px 表示なら 1.5 → 0.88px）。表示サイズで割り戻して、どの大きさでも同じ太さに見えるようにしてある。

太陽だけは作りを変えている。

- **芯は塗り**。線で描くと直径 4px 前後で内側が潰れ、光条とつながって「※」のような塊になる
- **光条は 3.3 単位**。元は 2 単位（14px 表示で 1.2px）しかなく、丸い線端と相まって点になっていた

### 色の決め方

Claude の配色に寄せている。**暖かいニュートラル + テラコッタ**の 2 本立てで、青系は使わない。

| 役割 | ライト | ダーク |
| --- | --- | --- |
| 下地 | 暖かいアイボリー `#efece3` | 暖かい黒 `#131211` |
| 面 | ほぼ白の膜（実効 `#faf9f5` 前後） | アイボリーの薄い膜（実効 `#262523` 前後） |
| アクセント | 錆色 `#a04a29` | テラコッタ `#e0906c` |
| 正解 | セージ | セージ |
| 不正解 | ローズ | ローズ |
| ヒント | kraft（Anthropic の砂色） | kraft |

決めるうえで効いた制約が 3 つある。

- **ライトのアクセントだけ濃い。** 基準は Claude のテラコッタ `#d97757` だが、明るい下地の上では文字色として 4.5:1 に届かない。ライトは一段濃い錆色にし、膜（`--c-accent-soft`）と枠は `#d97757` のままにしてある。塗りと下地は同じ色なので、両テーマで印象は揃う。
- **不正解はローズまで色相を振る。** 全体が暖色なので、オレンジ寄りの赤にすると隣のアクセントと見分けが付かない。同じ理由で、不正解の帯は `warning`（砂色）ではなく `danger` を使う。以前は砂色だったが、下地のにじみに溶けて「エラーに見えない」状態だった。
- **正解はセージ。** ミント/青緑はこの配色から浮くので、暖色側の緑に寄せている。

`npm run contrast` で、主要な文字と背景の組み合わせ 16 通り（両テーマ）を**実際に描画されたピクセルから**測っている。トークンの値だけで計算するとガラス（半透明の面＋背後のにじみ）を無視してしまうため、スクリーンショットを読む方式にした。全件 4.5:1（WCAG AA）以上を通す。
...
章ページも問題ページと同じ形にした。**左に節の目次、右に本文**（最大 62rem）。目次は貼り付けてあり、**いま読んでいる節に帯が滑って追従**する。

判定は「画面上端から 140px より上に出た最後の節」。最下部まで来たときは最終節にする（短い節が一度も選ばれないまま終わるのを防ぐ）。スクロールは `requestAnimationFrame` で 1 フレームに 1 回だけ読む。

帯そのものは問題ページのサイドバーと同じ `layoutId` の使い回しで、動きの語彙を共有している。狭い画面では 1 列に戻り、目次が本文の先頭に来る（以前と同じ並び）。

**目次のリンクは `<a href="#節id">` にできない。** HashRouter では URL のハッシュがルートそのもの（`#/learn/1`）なので、`#節id` に書き換えると別ページへ遷移してしまう。そのため目次はボタンにして、位置合わせを JS で行っている。

- 距離なりに **0.24〜0.44 秒**。長い章でも待たされない範囲に収める
- 固定ヘッダのぶん **80px 上を空けて**止める
- 着いたら節に `focus()` を移す。キーボード操作の続きがその節から始まる
- 途中でホイール・タッチ・キーが来たら**引き下がる**。`stop()` だけでは間に合わないことがあるので、`onUpdate` の書き込み側でも中止フラグを見ている
- 「視差効果を減らす」設定では一気に飛ぶ
...
| 場所 | 動き | 意図 |
| --- | --- | --- |
| 問題一覧のフィルタ | 残った行が滑って詰まる（`layout` + `popLayout`） | 52 行が瞬時に入れ替わると、何が減ったのか分からない |
| 選択式の採点 | 正解を 3px 持ち上げ、丸印を一拍ふくらませる | 色が変わるだけだと、正解を探し直すことになる |
| 選択式で外したとき | 選んだ選択肢を ±3px で小さく振る | 外したことを一目で伝える。振り幅は控えめにする |
| ホームのフェーズ別バー | 0 から順に伸びる（40ms ずつずらす） | 7 本を一度に見比べられるようにする |

消える行は `mode="popLayout"` で席を先に空けている。消える行のフェードと残る行の移動が同時に起きると、どちらも読み取れない。

これらは `npm run smoke` で**途中の位置**を測って検査している（終わった位置だけ見ても、動いたのか瞬間移動したのか区別が付かない）。ホームのバーだけは読み込み直後に測ると伸び終わっていることがあるので、`addInitScript` でページの JS より先に観測者を仕込み、`scaleX` の最小値を拾っている。
